...
build:
	...
	cd cmd/hexbridge && env GOARCH=arm64 CGO_CFLAGS=-Wno-undef-prefix go build -o ../../build/hexbridge $(VERSION)

install:
	@echo "  >  \033[32mInstalling bridge...\033[0m "
	cd cmd/hexbridge && go install $(VERSION)

build-mkdocs:
	docker run --platform amd64 --rm -it -v ${PWD}:/docs squidfunk/mkdocs-material build
...
test-e2e:
	...
	env CGO_CFLAGS=-Wno-undef-prefix go test -v -timeout 0 ./e2e

test-eth:
	@echo "  >  \033[32mRunning ethereum tests...\033[0m "
	env CGO_CFLAGS=-Wno-undef-prefix go test ./chains/ethereum

test-config:
	@echo "  >  \033[32mRunning config tests...\033[0m "
	env CGO_CFLAGS=-Wno-undef-prefix go test ./config
