...
import (
	...
	"hexbridge/chains/ethereum"
	"hexbridge/config"
	"hexbridge/utils/core"
	"hexbridge/utils/metrics/health"
	"hexbridge/utils/msg"
	"net/http"
	"os"
	"strconv"
...
var accountCommand = cli.Command{
	...
	Description: "The accounts command is used to manage the bridge keystore.\n" +
		"\tTo generate a new account (key type generated is determined on the flag passed in): hexbridge accounts generate\n" +
		"\tTo import a keystore file: hexbridge accounts import path/to/file\n" +
		"\tTo import a geth keystore file: hexbridge accounts import --ethereum path/to/file\n" +
		"\tTo import a private key file: hexbridge accounts import --privateKey private_key\n" +
		"\tTo list keys: hexbridge accounts list",
...
func init() {
	app.Action = run
	app.Copyright = "Copyright 2022 Hexlant"
	app.Name = "hexbridge"
	app.Usage = "HexBridge"
	app.Authors = []*cli.Author{{Name: "Hexlant"}}
	app.Version = Version
...
//	//defer db2.Close()
//	if err != nil {
//		log.Error("failed connectDB ", "Error = ", err)
//	}
//
//	db.AutoMigrate(&Deposit{})
//	db.AutoMigrate(&Proposal{})
//	log.Info("db info ", "db = ", db.Config)
//}
